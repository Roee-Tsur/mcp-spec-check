...
- **Phase D:** `docs/scan-2026-07.md` is a DRAFT — all rule-6/8/9 prose written, numbers are `‹TK: aggregates.json path›` placeholders with a finalize checklist at the top. Not yet linked from README.
- **Still pending (user-gated):** (1) the full overnight scan (~7h, bounded by the pipeworx serial queue — ~7h of outbound probing, so user-triggered) → fills `docs/scan-2026-07.aggregates.json` + the writeup numbers; (2) cutting the **v0.2.0** release (`npm version minor` → `git push --follow-tags`); (3) external cross-post at publish.
