...
```bash
git clone https://github.com/Roee-Tsur/mcp-ready.git
cd mcp-ready
npm install
npm test
...
1. **Wrong verdicts.** If `mcp-ready` gives your server a wrong verdict (fails a ready server, or passes one that isn't), open an issue with the `--json` output and, if possible, what your server runs. Probe correctness beats new features.
